# Getting Started with Slow Code kata in Python
...
`File` > `Open` > `Kata-Slow-Code` > `python`
...
Choose `Open Folder`, navigate to `Kata-Slow-Code` / `python` then click `Select Folder`.
